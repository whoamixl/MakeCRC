...
else
{
    ...
    {
        sprintf(s,"These are the %d  command- line  arguments passed  to main:/n\n", argc);
        f.write(s,strlen(s));
        qDebug()<<s;
        for(i=0;i<argc;i++)
        {
    ...
    //  qDebug()<<m_strReadBuffer.toHex();
    b = m_strReadBuffer.toHex();
    int n = b.length();
    while (n-2>0)
    {
        n=n-2;
     ...
     qDebug()<<b.size();
      ...
      if(crc_type == "crc16")
      {
        ...
        //sprintf(buffer,"%02X%02X", tmp[0], tmp[1]);
        qDebug()<<"temp0:"<<tmp[0];
        qDebug()<<"temp1:"<<tmp[1];
        //n = tohex16.toInt();
        //file.write(&ac);
        //file.write(&bc,strlen(&bc));
        //qDebug()<<&ac;
        //qDebug()<<&bc;
        //file.write(&bc);
        //file->write((qint32)tohex16);
        //qDebug()<<hex16;
        //qDebug()<<tohex16;
        }
        delete [] data_buff;
        ...
        else if(crc_type == "crc32")
        {
         crc32_in = crc32(m_strReadBuffer);
         qDebug() <<crc32_in;
         QString crc32_hex = QString::number( crc32_in, 16 );

         char Lch[5]= {crc32_in>>24,
                       crc32_in>>16,
                       crc32_in>>8,
                       crc32_in,
                       '\0'};
